...
def get_naive_mask(first_frame, hsv_frame, thresh=0.03):
    # start = time.time()
...
# takes the first few frames (e.g. 5 seconds of frames) and configure the threshold
def configure(configure_frames):
    mean = np.mean(configure_frames, axis=0)
    std = np.std(configure_frames, axis=0)
    # print(std[10000: 10100])
    return mean, std # shape is (frame.shape[0] * frame.shape[1], frame.shape[1])
...
def main():
    ...
    while True:
        ...
        if count <= 240:
            ...
            # print(first_hsv_frame.reshape((hsv_frame.shape[0] * hsv_frame.shape[1], hsv_frame.shape[2])))
            mask = get_stat_frame(mean, std, hsv_frame)
        ...
        # if keyboard == 'q' or keyboard == 27 or count > 1000:
        if keyboard == 'q' or keyboard == 27:
            break

    out = cv.VideoWriter('stat_mask.mp4', cv.VideoWriter_fourcc(*'mp4v'), 30, size) # last parameter set color to gray scale
