...
//        }
    ...
    public unsafe static class FrameProcessor
    {
        ...
        public static void ProjectionMasking(SpatialCoordinateSystem frameCoordinateSystem, VideoMediaFrame videoMediaFrame, byte* camera_frame, int camera_start_index, int camera_width, int camera_height)
        {
            ...
            if (is_first_frame)
            {
                first_frame = bgra2hsv(target_frame, targetWidth, targetHeight);
//#if ENABLE_WINMD_SUPPORT
...
//#endif
        ...
        {
            float[] result = new float[frameWidth * frameHeight * 3];
            for (int i = 0; i < frameWidth * frameHeight; i++)
            {
                float B = bgra_frame[i * 4] / 255.0f;
                float G = bgra_frame[i * 4 + 1] / 255.0f;
                float R = bgra_frame[i * 4 + 2] / 255.0f;

                float X_max = Math.Max(R, Math.Max(G, B));
                float X_min = Math.Min(R, Math.Min(G, B));
                float C = X_max - X_min;
        ...
        public static byte* Bg_subtraction(bool flip_Y=true, float thresh_h=0.042f, float thresh_s=0.8f, float thresh_v=0.8f)
        {
            if (first_frame == null)
            {
                Debug.Log("first frame (background) not defined!");
                return null;
            }
            else
            {
